...
| **LFM2.5 230M · WebGPU kernels** | ~140 MB | Custom WGSL (in-tab) | Text + agent | LFM2.5 230M on a from-scratch WebGPU engine — every kernel hand-written WGSL; ~670 tok/s on an M4 Pro, ~8× the same model on ONNX. **Default model.** |
| **Gemma 4 E2B · WebGPU kernels** | ~2 GB | Custom WGSL (in-tab) | Text + agent | Gemma 4 E2B (Google's QAT int4) on hand-written WGSL kernels; ~57 tok/s on an M4 Pro, 3.6× the ONNX Gemma 4 E2B. Text-only — pick the ONNX E2B for images and audio |
