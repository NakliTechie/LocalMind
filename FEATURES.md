...
| **LFM2.5 230M · WebGPU kernels** | ~140 MB | Custom WGSL (in-tab) | Text + agent | LFM2.5 230M on a from-scratch WebGPU engine — every kernel hand-written WGSL; ~1,060 tok/s on an M4 Pro, 3.6× the same model on ONNX. **Default model.** |
| **Gemma 4 E2B · WebGPU kernels** | ~2 GB | Custom WGSL (in-tab) | Text + agent | Gemma 4 E2B (Google's QAT int4) on hand-written WGSL kernels; ~170 tok/s on an M4 Pro, 4× the ONNX Gemma 4 E2B. Text-only — pick the ONNX E2B for images and audio |
| **Ternary Bonsai 2 27B** | ~5.9 GB | Custom WGSL (in-tab) | Text + agent | 27B at ternary 1.75-bit (Qwen3.8 backbone, 98% of full-precision benchmark score) on hand-written WGSL — the biggest in-browser model here. A reasoning model; 16K context. Speculative decoding (DFlash 2, below) on by default |
...
## Runtimes — seven engines, five ways to run a model
...
3. **Custom WebGPU kernels (in-tab)** — purpose-built, from-scratch WebGPU inference engines where *every* kernel is hand-written WGSL — no ONNX runtime, no llama.cpp. Three are bundled: **LFM2.5 230M** (`Lfm2Mobile`, from `webml-community/lfm2-webgpu-kernels` — RoPE, RMSNorm, Q4_0 dequant, the LFM2 short-conv, GQA attention), **Gemma 4 E2B** (`Gemma4Mobile`, from `webml-community/gemma-4-webgpu-kernels` — QAT int4 matmul, embed-gather-norm, RoPE/RMSNorm, GQA + sliding-window attention), and **Ternary Bonsai 2 27B** (`TernaryBonsai2`, from `webml-community/ternary-bonsai-2-webgpu-kernels` — PTQ1_0 ternary on-the-fly dequant, the Gated-DeltaNet linear-attention recurrence, RoPE/RMSNorm, SwiGLU, GQA attention). Each reads its quantized weights directly and is tuned for maximal decode throughput. WebGPU-only (no CPU fallback).
4. **Your own local server (endpoint)** — point LocalMind at an OpenAI-compatible server on your machine (Ollama, LM Studio, llama.cpp, Atomic) in Settings → Models. The model runs on the server at native speed; the browser just streams. Lets you use big models (7B–70B+) and the whole Ollama / LM Studio library. Still local (localhost) — nothing leaves the device. A preset menu fills the URL (Ollama, LM Studio, llama.cpp, Atomic, and *Ternary Bonsai 2 — fast (Mac, dflash serve)* at `http://127.0.0.1:8790/v1`); a per-endpoint **temperature** (0–2, default 0.7) is stored with the URL — set 0 for lossless speculative decoding on `dflash serve`, which only speculates at greedy. From the deployed https origin Chrome gates loopback / LAN fetches behind a *Local network access* prompt; if a request fails at the network level the error names that prompt.
...
**Speculative decoding for Ternary Bonsai 2 (DFlash 2).** Settings → Models → *Speculative decoding for Ternary Bonsai 2 27B* (on by default). After the model is ready, LocalMind fetches a 1.1 GB block drafter once — [naklitechie/Qwen3.8-27B-DFlash2-ternary-bonsai2](https://huggingface.co/naklitechie/Qwen3.8-27B-DFlash2-ternary-bonsai2), z-lab's DFlash 2 drafter re-fitted on the ternary model's own output — keeps it packed on the GPU (~1 GB) and lets the 27B model verify a 5-token block per step through a small-batch WGSL GEMM instead of decoding one token at a time. Output is greedy-identical to plain decode; the status line shows `· speculative` once it is active, and turns that start before the drafter lands decode plainly. Measured in-app on an M4 Pro: 34.4 vs 40.7 ms/token on a 1,486-token code answer (1.18×, 4.1 tokens per verify cycle). On a quiet machine through the JavaScript API (256 tokens, greedy): 28.8 → 42.9 tok/s on a code prompt (1.49×, output byte-identical to plain decode), and no gain on prose (27.2 → 27.4). The engine patches (`scripts/bonsai2-dflash-patches.mjs`) and the runner module (`ternary_bonsai_2_dflash.js`, built by `scripts/build-bonsai2-dflash.mjs` from [NakliTechie/dflash-mlx-bonsai2](https://github.com/NakliTechie/dflash-mlx-bonsai2)) are the same work as the MLX version behind the *Ternary Bonsai 2 — fast (Mac, dflash serve)* endpoint preset.
...
Most "run an LLM in your browser" projects are **chat UIs for a model**. LocalMind is a **research agent with persistent memory, tools, web enrichment, image + text-diffusion generation, and seven interchangeable engines (five kinds of runtime)** — in a single HTML file with zero build.
