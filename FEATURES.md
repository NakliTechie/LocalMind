...
3. **Custom WebGPU kernels (in-tab)** — purpose-built, from-scratch WebGPU inference engines where *every* kernel is hand-written WGSL — no ONNX runtime, no llama.cpp. Two are bundled: **LFM2.5 230M** (`Lfm2Mobile`, from `webml-community/lfm2-webgpu-kernels` — RoPE, RMSNorm, Q4_0 dequant, the LFM2 short-conv, GQA attention) and **Ternary Bonsai 2 27B** (`TernaryBonsai2`, from `webml-community/ternary-bonsai-2-webgpu-kernels` — PTQ1_0 ternary on-the-fly dequant, the Gated-DeltaNet linear-attention recurrence, RoPE/RMSNorm, SwiGLU, GQA attention). Each reads its quantized weights directly and is tuned for maximal decode throughput. WebGPU-only (no CPU fallback).
...
Most "run an LLM in your browser" projects are **chat UIs for a model**. LocalMind is a **research agent with persistent memory, tools, web enrichment, image + text-diffusion generation, and five interchangeable runtimes** — in a single HTML file with zero build.
