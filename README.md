...
The Bonsai family are 1.58-bit ternary-weight LLMs from Prism ML (Apache-2.0, Qwen3 backbone). They benchmark materially higher than Gemma 3 1B on reasoning/code/tool-calling at roughly half the download size. Qwen3 4B is the standard 4-bit-quantized Qwen3-4B from Alibaba (Apache-2.0) — heavier than Bonsai 4B but trades the ternary-quantization quality cost for stock-recipe weights. LFM2 8B A1B is Liquid AI's sparse mixture-of-experts (8B total parameters, ~1B active per token) at q4f16. **Liquid AI's own official ONNX model card flags the 8B-A1B as "too large for WebGPU browser inference" — yet here it is, running fully on-device in your browser, tool calling and all.** The catch is _which_ export you load: the asymmetric/zero-point QMoE builds (including Liquid's official `LFM2.5-8B-A1B-ONNX`) genuinely don't load on onnxruntime-web's WebGPU backend, but the stock **symmetric** QMoE export (`onnx-community/LFM2-8B-A1B-ONNX`) does. It's WebGPU-only and ~4.8 GB, so it wants a capable GPU and ~8 GB+ RAM. Multimodal input (images/audio) still goes through Gemma 4.
