...
6. **Bonsai 27B WebGPU kernels** (`backend: 'bonsai27b-webgpu'`) — the same pattern as (4)/(5) for the 1-bit **Bonsai 27B** (Qwen3.6-27B backbone, hybrid Gated-DeltaNet + full attention): a from-scratch WebGPU engine (`bonsai_27b.js`, exporting `Bonsai27bMobile`) vendored from `webml-community/bonsai-webgpu-kernels`, with hand-written WGSL for Q1_0_g128 on-the-fly dequant, the linear-attention recurrence, RoPE / RMSNorm / SwiGLU / GQA attention, reading a Q1_0 GGUF directly (~3.8 GB) from the ungated Apache-2.0 `lmstudio-community/Bonsai-27B-GGUF` mirror. Its worker (`#bonsai27bWebgpuWorkerSrc`) passes `{role, content}` straight through (Qwen3.6 has a real system role and templates internally) and reuses the same shared protocol as (4). Vendored engine ships no upstream license (consistent with the other three); model weights are Apache-2.0.
...
- **[Bonsai 27B](https://huggingface.co/lmstudio-community/Bonsai-27B-GGUF)** — 1-bit (Q1_0) Qwen3.6-27B, custom WebGPU kernels engine (`bonsai_27b.js`), ~3.8 GB, Apache-2.0 weights
...
`lfm2_5.js` (~650 KB) and `gemma-4-e2b.js` (~540 KB). Everything else loads
from CDN with SRI where possible.

Deploy by serving `index.html`, `inference-worker.js`, `lfm2_5.js`, and
`gemma-4-e2b.js` together from any static host. GitHub Pages, Netlify, S3, or
